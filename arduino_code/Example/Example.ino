...
void loop() {
  if (start) {
    ...
    attachInterrupt(
        2, startProgramm,
        CHANGE); // Снова разрешили прерывание по нажатию кнпоки "ПУСК"
  }
  delay(500);
}
...
void readWriteAllValues(){
  for (int i = 0; i < 17; i++){
    ...
    while (!valueWasReceived);  // Опасный момент, можем зависнуть здесь
навсегда
  }
}
*/
...
void startProgramm() {
  if (!start)
    start = true;
  detachInterrupt(
      2); // Запретили на всякий случай прервание по нажатию кнопки "ПУСК"
}
